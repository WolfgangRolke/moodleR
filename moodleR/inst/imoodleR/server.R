...
shinyServer(function(input, output, session) {
   ...
   output$defineInputs=renderUI({
      ...
      for(i in 1:n)
        out[[i]] =tagList(
          ...
          textAreaInput(paste0("calculate.answer",i), "Enter Calculation of Answer", placeholder=""),    
          textAreaInput(paste0("atxt",i), "", placeholder=" ", width="100%"),
   ...
   qtxt = eventReactive(input$xmlbutton,{
        ...
        for(i in 1:input$numquestions) { 
            ...
            if(get.info()[[i]]$answertype=="Numeric Answer")   {
                ...
                question[i] = paste0("moodleR::nm(res[[", i, "]], w = ", w , ", eps = ", eps, ")")
            } 
            if(get.info()[[i]]$answertype=="Multiple Choice")   { 
              ...
              question[i] = paste0("moodleR::mc(", options,  ", ifelse(1:", m, "== res[[", i, "]], 100, 0))$qmc")
            }
            if(get.info()[[i]]$answertype=="Numeric Matrix") {
                ...
                eps=get.info()[[i]]$numprecision
                if(length(eps)>1) eps=paste0("c(", paste0(eps, collapse=","),")")             
                question[i] = paste0("moodleR::qamatrix(res[[", i, "]], points=", w, ", precision=", eps, ")$qtxt")
            }
   ...
   atxt = eventReactive(input$xmlbutton,{
        ...
        for(i in 1:input$numquestions) {
            ...
            if(get.info()[[i]]$answertype=="Numeric Matrix") 
                answer[i] = paste0("moodleR::qamatrix(res[[", i, "]])$atxt")
            if(get.info()[[i]]$answertype == "Multiple Choice") { 
                m=length(get.info()[[i]]$options)
                options=paste0("\"", get.info()[[i]]$options,"\"")
                options=paste0("c(", paste0(options, collapse=","),")")
   ...
   output$text <- renderPrint({                   
        ...
        if(input$doquiz=="Yes")
           moodleR::genquiz(as.numeric(input$numquiz), fun, folder = folder())     
